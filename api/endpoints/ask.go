...
// @Router /ask  [post]
func ProceedAsk(w http.ResponseWriter, r *http.Request) {
	utils.SetupCORS(&w)
	if (*r).Method == "OPTIONS" {
		return
	}
	...
	store, err := storage.NewSeparatedMemoryStorage("./data/" + question.Corpora + ".gob")
	if err != nil {
		log.Println(err)
	}
	...
	response.Set("answerTime", time.Since(startTime).Microseconds())
